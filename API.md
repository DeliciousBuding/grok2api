...
Admin token tag inputs accept at most 10 unique non-empty tags per account and each tag must be at most 64 characters after trimming. `POST /admin/api/tokens`, `POST /admin/api/tokens/add`, and `PUT /admin/api/pool` reject invalid tags with `too_many_tags` or `tag_too_long` before storage or background refresh work starts. Text and SQLite account storage enforce the same bounds on create, replace, and tag patch writes.
