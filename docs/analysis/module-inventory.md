...
| `internal/api` | HTTP routes, auth, request parsing, OpenAI/Anthropic compatibility | Handlers are large and mix parsing, retries, upstream calls, and response formatting; admin token listing now has an explicit pagination contract. | High |
...
5. Admin batch/cache paths still need the same negative-case depth now added to token listing.
