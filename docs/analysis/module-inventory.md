...
| `internal/api` | HTTP routes, auth, request parsing, OpenAI/Anthropic compatibility | Handlers are large and mix parsing, retries, upstream calls, and response formatting; admin token, batch/cache, and asset management paths now have explicit validation contracts. | High |
...
5. Destructive admin paths now fail fast on malformed input; audit logging for those actions remains future work.
