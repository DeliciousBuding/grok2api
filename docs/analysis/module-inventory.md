...
| `internal/api` | HTTP routes, auth, request parsing, OpenAI/Anthropic compatibility | Handlers are large and mix parsing, retries, upstream calls, and response formatting; admin token listing and batch/cache parameter validation now have explicit contracts. | High |
...
5. Asset-management paths still need the same pagination and negative-case depth now added to token listing and cache management.
6. Config reload must be safe and predictable.
