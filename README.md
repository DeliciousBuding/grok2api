...
[account.storage]
backend = "text"                # text/jsonl/local、sqlite 或 postgres；pg+redis 当前会 fail-fast
...
如需使用共享 PostgreSQL，可切换到 `postgres` 后端：
...
也可以通过容器环境变量设置 `ACCOUNT_STORAGE_BACKEND=postgres` 和 `ACCOUNT_POSTGRES_DSN=...`。PostgreSQL 后端使用 `accounts` + `account_meta` 表保存账号 JSON 记录和单调 revision，适合单活或低并发多实例共享账号池。`pg+redis` / `postgres+redis` / `postgresql+redis` 仍是预留的分布式账号后端名称，当前版本会在启动时 fail-fast，不会静默回退到 JSONL、SQLite 或裸 PostgreSQL。
...
>
> **指定版本**：`docker pull ghcr.io/deliciousbuding/grok2api:v1.0.4`，或用 commit 短哈希 `ghcr.io/deliciousbuding/grok2api:<sha>` 锁定具体构建。
>
